...
const Dashboard = () => {
    ...
    const handleJoinTeam = async () => {
        const targetTeamId = window.prompt("Enter the Tactical Unit ID to join:");
        if (!targetTeamId) return;
        
        try {
            navigate(`/invite/${targetTeamId}`);
        } catch (err) {
                                                ...
                                                {messages.length > 0 ? messages.map((msg, i) => (
                                                    <div key={msg.id || i} className={cn("flex flex-col", msg.senderId === user.uid ? "items-end" : "items-start")}>
                                                         <div className="flex items-center gap-2 mb-1">
                                                            ...
                                                            <span className="text-[7px] text-slate-500">{msg.dateObject ? format(msg.dateObject, 'HH:mm') : 'Sync...'}</span>
                                                        </div>
                                                        <div className={cn(
